...
public class B_FileIO {
    // NOTE GB01
    // IO (Input, Output)
    // 입출력 : 외부자원으로부터 프로그램으로 데이터를 읽어오거나, 외부자원으로 데이터를 내보내는 것
    ...
    // NOTE GB03 : Stream 구분
    // 1. Byte 기반 Stream (1 byte 데이터를 수송신)
    // 2. 문자 기반 Stream (2 byte 데이터를 수송신)

    // a. 기반 Stream : 실제 외부자원과 데이터를 입출력하는 Stream
    // b. 보조 Stream : 기반 Stream에 기능을 추가해주는 데코레이터
    //                 FilterStream을 상속받는다. >> FilterStream을 상속받으면 보조 Stream 이구나!
