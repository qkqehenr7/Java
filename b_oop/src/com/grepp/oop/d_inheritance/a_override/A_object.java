...
public class A_object {
    ...
    // NOTE DA02 : override
    // override : 재정의, 부모 클래스로부터 상속받은 메서드를 자식 클래스에서 재정의하는 것
    ...
    // @Override : 이 메서드는 부모 클래스로부터 상속받은 메서드를 override 한 메서드임을 컴파일러에게 전달
    // 컴파일러는 부모 클래스에 이 메서드와 반환타입, 매개변수, 이름이 일치하는 메서드가 있는지 확인 후 없으면 에러
    @Override
    ...
    // SHA-256 해시함수 : 어떤 데이터를 256bit 의 데이터로 변환하는 함수

    // 같은 객체라면 언제나 같은 해시값을 가져야하기 때문에
    // 객체의 같음을 판단하는 equals 가 override 되었다면, hashCode 가 함께 Override 되어야 한다.
    @Override
    public int hashCode() {
